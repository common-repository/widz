 ...
 */
abstract class Widz_Widget extends WP_Widget {
	...
	public function registered_fields( $hook ) {
		if ( $hook == 'widgets.php' ) {
			...
			if ( empty( $widz_registered_fields ) ) {
				$widz_registered_fields = array();
			}

			foreach ( $this->fields as $key => $field ) {
				$widz_registered_fields[] = $field['type'];
			}
		}
	}
	...
	public function get_cached_widget( $args ) {
		...
		$cache = wp_cache_get( apply_filters( 'widz_cached_widget_id', $this->widget_id ), 'widget' );
	...
	public function cache_widget( $args, $content ) {
		wp_cache_set( apply_filters( 'widz_cached_widget_id', $this->widget_id ), array( $args['widget_id'] => $content ), 'widget' );
	...
	public function flush_widget_cache() {
		wp_cache_delete( apply_filters( 'widz_cached_widget_id', $this->widget_id ), 'widget' );
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $this->fields as $field ) {
			 ...
			 */
			$value = apply_filters( "widz_sanitize_field_{$field['type']}", $value, $new_instance[$field['name']] );
	...
	public function form( $instance ) {
		...
		$this->field_wrapper = apply_filters( 'widz_widget_field_wraper', '<div class="widz_widget_row"><div class="col-label">%1$s</div><div class="col-field">%2$s</div></div>' );
		...
		foreach ( $this->fields as $field ) {
			 ...
			 * Print field
			 */
			if ( function_exists( "widz_form_{$field['type']}" ) ) {
				...
				$output.= sprintf( $this->field_wrapper, $lable, call_user_func( "widz_form_{$field['type']}", $field, $value ) . $desc );
			} else if ( has_filter( "widz_form_{$field['type']}" ) ) {
				$field['value'] = $value;
				$field_output = apply_filters( "widz_form_{$field['type']}", '', $field );
